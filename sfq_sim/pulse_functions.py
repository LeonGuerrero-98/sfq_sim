...
def sfq_qutrit_Ry_jitter_sweep(n, anharm, omega_10,initial_state,theta,jitter_sigmas,averaging = 1,pulse_width = 2e-12,t_delay = 1e-11, n_steps = 3e5, progress = False):
    ...
    for sigma in jitter_sigmas:
        for i in range(averaging):
            t,pulse = jitter_sfq_int(n,omega_10,sigma,pulse_width,t_delay,n_steps) # Generating pulse signal
            pulse_func = sp.interp1d(t,pulse,fill_value = "extrapolate") # Interpolating pulse function
            ...
            result = sesolve(H_t, psi0, t, e_ops=[target_state_op, state2_op, state1_op, state0_op], options={"max_step": pulse_width/3, "progress_bar": False}) # Solve for coefficients of each level. Max step must be < 1/2 pulse width, otherwise will lead to incorrect solution
            #only store final result of evolution
            fids.append(result.expect[0][-1]) 
            P2.append(result.expect[1][-1]) 
            P1.append(result.expect[2][-1])
        ...
        #fids in first return is equal to P1 for this gate
        fids_mean.append(np.mean(P1))
        ...
        result = {"jitter_sigmas": jitter_sigmas, "fids": fids_mean, "P2": P2_mean, "P1": P1_mean, "P0": P0_mean, "fids_err": fids_err, "P2_err": P2_err, "P1_err": P1_err, "P0_err": P0_err}
    return result
